...
def convert_weights(old_weights_path, config_path, charset_path, output_path):
    ...
    old_state = torch.load(old_weights_path, map_location="cpu", weights_only=False)
    ...
    # Конвертируем ключи: attn. -> attention_decoder.
    print("\nConverting key names...")
    converted_state = {}
    for key, value in old_state.items():
        if key.startswith("attn."):
            new_key = key.replace("attn.", "attention_decoder.")
            converted_state[new_key] = value
            print(f"  {key} -> {new_key}")
        else:
            converted_state[key] = value

    # Загружаем конвертированные веса
    print("\nAttempting to load weights...")
    try:
        missing_keys, unexpected_keys = model.load_state_dict(
            converted_state, strict=False
        )
        ...
        if not missing_keys and not unexpected_keys:
            ...
            cnn_loaded = all(
                k.startswith("cnn.") for k in missing_keys if k.startswith("cnn.")
            )
            enc_loaded = all(
                k.startswith("enc_rnn.")
                for k in missing_keys
                if k.startswith("enc_rnn.")
            )
            attn_loaded = all(
                k.startswith("attn.") for k in missing_keys if k.startswith("attn.")
            )
...
def main():
    # Пути к файлам
    old_weights = r"C:\Users\pasha\OneDrive\Рабочий стол\trba_base_g0.pth"
    config_path = r"src\manuscript\recognizers\_trba\configs\config.json"

    # Используем charset из пакета
    current_dir = (
        Path(__file__).parent.parent / "src" / "manuscript" / "recognizers" / "_trba"
    )
    charset_path = current_dir / "configs" / "charset.txt"

    output_path = r"trba_base_g0_converted.pth"
    ...
    if success:
        ...
        print("\nNext steps:")
        print(
            f'1. Test with: python scripts/trba_infer_speed_test.py --model-path "{output_path}" --config-path "{config_path}" --folder <test_folder> --gt-csv <test_csv>'
        )
